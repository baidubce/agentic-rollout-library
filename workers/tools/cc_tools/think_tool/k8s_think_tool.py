...
class K8sThinkTool(CCToolBase):
    ...
    def get_openai_tool_schema(self) -> OpenAIFunctionToolSchema:
        return create_openai_tool_schema(
            name="Think",
            description="Record a thought payload for analysis.",
            parameters={
                "thought": {
                    "type": "string",
    ...
    async def execute_tool(self, instance_id: str, parameters: Dict[str, Any], **kwargs) -> ToolResult:
        t0 = time.time()
        try:
            if "thought" not in parameters:
                raise ValueError("Invalid input: 'thought' is required")
            thought = parameters["thought"]
            if not isinstance(thought, str):
                raise ValueError("Invalid input: 'thought' must be a string")
            payload = base64.b64encode(thought.encode("utf-8")).decode("ascii")
            user_cmd = f'echo {shlex.quote(payload)} | base64 -d >/dev/null 2>&1; echo "ACK:think"'
            ...
            logger.exception("K8sThinkTool execution failed")
